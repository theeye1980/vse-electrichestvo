<?php
/*
Plugin Name: Вес кабеля
Plugin URI: https://wpcalc.com/ves-kabelya/
Description: После активации плагина, вставьте шорткод [wpcalc-296] там где хотите отобразить калькулятор
Version: 2.0
Author: Lobov Dmitriy
Author URI: https://wpcalc.com
*/
function show_wpcalc_296 () {
	 $lang = 'ru';
	 	
	
	 $file = plugin_dir_path(__FILE__).'calc/296.php'; 
     $scipt = plugin_dir_path(__FILE__).'js/296.js';
     $css = plugin_dir_path(__FILE__).'css/296.css';	 
	 ...
	 if(file_exists($scipt)) {
		 wp_enqueue_script( 'wpcalc-script', plugins_url( '/js/296.js', __FILE__ ));
	 }
	 if(file_exists($css)) {
		 wp_enqueue_style( 'wpcalc-css', plugins_url( '/css/296.css', __FILE__ ));
	 }
	 	 	 
	 return $calc;
}
add_shortcode('wpcalc-296', 'show_wpcalc_296');