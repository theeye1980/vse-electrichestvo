<?php
/*
Plugin Name: Ток в цепи
Plugin URI: https://wpcalc.com/tok-v-cepi/
Description: После активации плагина, вставьте шорткод [wpcalc-760] там где хотите отобразить калькулятор
Version: 2.0
Author: Lobov Dmitriy
Author URI: https://wpcalc.com
*/
function show_wpcalc_760 () {
	 $lang = 'ru';
	 	
	
	 $file = plugin_dir_path(__FILE__).'calc/760.php'; 
     $scipt = plugin_dir_path(__FILE__).'js/760.js';
     $css = plugin_dir_path(__FILE__).'css/760.css';	 
	 ...
	 if(file_exists($scipt)) {
		 wp_enqueue_script( 'wpcalc-script', plugins_url( '/js/760.js', __FILE__ ));
	 }
	 if(file_exists($css)) {
		 wp_enqueue_style( 'wpcalc-css', plugins_url( '/css/760.css', __FILE__ ));
	 }
	 	 	 
	 return $calc;
}
add_shortcode('wpcalc-760', 'show_wpcalc_760');