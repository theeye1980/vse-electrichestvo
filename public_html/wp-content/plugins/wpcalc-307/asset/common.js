...
if(ifrmHeight=="")
{ifrmWidth="500";}
$("#embdTxt").val("<iframe src='"+embedUrl+"' width='"+ifrmWidth+"' height='"+ifrmHeight+"' frameborder='0'></iframe>");$("#embdTextarea").show();}
function easyRoundOf(value,decima)
{decima=Math.pow(10,decima);var rndvalus = Math.round(value * decima) / decima;
if(isNaN(rndvalus)){return 0;}else{return rndvalus;}}
...
$('.wpcalc').find("input[type = 'text']").focusout(function() {$("#dynErrDisp").html("");$("#dynErrDisp").hide();var buttonValue = $('.wpcalc').find("input[type = 'button']").val();if (buttonValue == "Calculate" || buttonValue == "calculate" || buttonValue == "Convert" || buttonValue == "Result"){
...
var buttonValue = $('.wpcalc').find("input[type = 'button']").val();if (buttonValue != "")
{var butnOnclikFunName = $('.wpcalc').find("input[type = 'button']").attr("onclick");var funStr = butnOnclikFunName.replace("();","");
funStr = funStr.replace("()","");var funcStrConv = eval(funStr).toString().strcontains(".php");
if(funcStrConv === false){setTimeout(function(){eval(butnOnclikFunName)}, 500);setTimeout(function(){$("input[type = 'text']").focus(function(){return false;});0==repeatErr&&($(".wpcalc").find("input[type = 'button']").after('<div class="errClass" id="dynErrDisp"></div><div id="calciScroll"></div>'),repeatErr=1);}, 1);}}
ga('send', 'event', 'text', 'click', 'Рассчет');
});
});
...
var easyValid=0;var repeatErr=0;function easyCommonInputCalculation(textid,unitname,alertmsg,arind)
{if(easyValid==0)
{var inputval=emptyValidation($("#"+textid+"hname").val(),alertmsg);if(inputval!=false||inputval==0)
{if (arind == "" || typeof(arind) == "undefined"){arind =0;}else {arind = arrayKeyFinder(unitname,arind);}return getbasicval((textid+"unit"),inputval,unitname,'',arind);}}
else{return false;}}
...
easyValid = 0;return parseFloat(0);}
else{easyValid=0;
if(isNaN(val)){return 0;}else{return parseFloat(val);}}}
$(function()
...
{setTimeout(function(){eval($(".wpcalc select").attr("onchange"))},100);}});$(":button").click(function()
...
var EasyMath={fact:function(num)
{Math.round(num);if(num<0)
return"Undefined";var fact=1;for(var i=num;i>1;i--)
fact*=i;return fact;},gamma:function(num)
{Math.round(num);if(num<1)
return"Undefined";var gamma=EasyMath.fact(num-1);return gamma;},arcosh:function(num)
{var arcosh=Math.log(num+Math.sqrt(num*num-1));return arcosh;},arrayTrim:function(string,chars)
{string=$.trim(string);if(string.charAt(0)==chars)
...
function mobileKeypad(){var e=$(window).width();if(480>e){var t=new Array(".easynumeric",".easyinteger",".easypositive",".easypositive-integer");$.each(t,function(e,t){$(t).prop("type","tel")}),$(".wpcalc").find("input[type = 'text']").each(function(){var e=$(this).attr("onkeyup");"undefined"!=typeof e&&"-1"!=e.indexOf("checnum(this)")&&$(this).prop("type","tel")})}}function mobileKeypadNumber(){var e=$(window).width();480>e&&$(".wpcalc").find("input[type = 'text']").each(function(){var e=$(this).attr("onkeyup");"undefined"==typeof e||"-1"==e.indexOf("checnum(this)")&&"-1"==e.indexOf("checnum1(this)")||$(this).prop("type","number")})}
