<?php
/*
Plugin Name: Заземление
Plugin URI: https://wpcalc.com/zazemlenie/
Description: После активации плагина, вставьте шорткод [wpcalc-307] там где хотите отобразить калькулятор
Version: 2.0
Author: Lobov Dmitriy
Author URI: https://wpcalc.com
*/
function show_wpcalc_307 () {
	 $lang = 'ru';
	 	
	
	 $file = plugin_dir_path(__FILE__).'calc/307.php'; 
     $scipt = plugin_dir_path(__FILE__).'js/307.js';
     $css = plugin_dir_path(__FILE__).'css/307.css';	 
	 ...
	 if(file_exists($scipt)) {
		 wp_enqueue_script( 'wpcalc-script', plugins_url( '/js/307.js', __FILE__ ));
	 }
	 if(file_exists($css)) {
		 wp_enqueue_style( 'wpcalc-css', plugins_url( '/css/307.css', __FILE__ ));
	 }
	 	 	 
	 return $calc;
}
add_shortcode('wpcalc-307', 'show_wpcalc_307');